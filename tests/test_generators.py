...
# Тестовый набор для transaction_descriptions
@pytest.mark.parametrize(
    "transactions, description, expected",
    [
        # Позитивные тесты
        (transactions, "Перевод организации", [transactions[0], transactions[4]]),
        (transactions, "Перевод со счета на счет", [transactions[1], transactions[2]]),
        (transactions, "Перевод с карты на карту", [transactions[3]]),

        # Негативные тесты
        (transactions, "", []),
        (transactions, "Неверный перевод", []),
        (transactions, "Перевод с карты на карту ", []),  # с пробелом в конце
...
def test_transaction_descriptions(transactions: List[Any], description: str, expected: List[Any]):
    # Получаем результат
    result = list(transaction_descriptions(transactions, description))
